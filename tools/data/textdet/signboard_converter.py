"""将COCO格式的 广告牌数据集, 制作好对应的annotation文件 (json 格式)"""
import argparse
import glob
...
def collect_files(img_dir, gt_dir):
    ...
    for img_file in imgs_list:
        gt_file = osp.join(gt_dir , osp.splitext(osp.basename(img_file))[0] + '.txt')
...
def load_img_info(files):
    ...
    anno_info = []
    for line in gt_list:
        # each line has one ploygen (4 vetices), and is_signboard flag
        # e.g., 695,885,866,888,867,1146,696,1143,1,
        line = line.strip()
        strs = line.split(',')
        category_id = 1
        xy = [int(x) for x in strs[0:8]]
        is_signboard = int(strs[-1])
...
def parse_args():
    parser = argparse.ArgumentParser(description='Convert signboard detection annotations to COCO format')
    parser.add_argument('--signboard_path', type=str,
        default="/Users/liuzhian/PycharmProjects/mmocr/tests/data/sample_data/1413_coco",
        help='signboard root path')
    parser.add_argument('-o', '--out-dir', help='output path')
    parser.add_argument(
        '--split-list',
        type=str,
        default="training test",
        help='a list of splits with space in between. e.g., "--split-list training test"')
    parser.add_argument(
...
def main():
    args = parse_args()
    signboard_path = args.signboard_path
    out_dir = args.out_dir if args.out_dir else signboard_path
    mmcv.mkdir_or_exist(out_dir)

    img_dir = osp.join(signboard_path, 'imgs')
    gt_dir = osp.join(signboard_path, 'annotations')

    set_name = {}
    for split in args.split_list.split(" "):
